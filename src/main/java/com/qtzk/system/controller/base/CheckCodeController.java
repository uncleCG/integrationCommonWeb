...
public class CheckCodeController extends BaseController {
	...
	public void getCheckCode(HttpServletRequest request, HttpServletResponse response) {
		...
		session.setAttribute(Constant.CHECKCODE_SESSION_KEY, random);
	...
	public String checkCode(HttpServletRequest request, String checkCode) {
		...
		result.put("status", 0);
		String exitsCheckCode = (String) request.getSession().getAttribute(Constant.CHECKCODE_SESSION_KEY);
		logger.debug("exitsCheckCode=>" + exitsCheckCode);
